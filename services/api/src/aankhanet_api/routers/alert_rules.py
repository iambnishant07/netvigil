...
async def list_alert_rules(current_user: _ReadRules, org: EffectiveOrg) -> list[AlertRuleOut]:
    async with db.get_connection() as conn:
        rules = await ar_repo.list_alert_rules(conn, org)
...
async def create_alert_rule(body: AlertRuleCreate, current_user: _WriteRules, org: EffectiveOrg) -> AlertRuleOut:
    async with db.get_connection() as conn:
        rule = await ar_repo.create_alert_rule(
            conn, org, body.name, body.min_severity,
...
async def patch_alert_rule(rule_id: str, body: AlertRuleUpdate, current_user: _WriteRules, org: EffectiveOrg) -> AlertRuleOut:
    async with db.get_connection() as conn:
        rule = await ar_repo.patch_alert_rule(
            conn, org, rule_id, **body.model_dump(exclude_none=True)
        )
    if not rule:
...
async def delete_alert_rule(rule_id: str, current_user: _WriteRules, org: EffectiveOrg) -> None:
    async with db.get_connection() as conn:
        deleted = await ar_repo.delete_alert_rule(conn, org, rule_id)
    if not deleted:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Rule not found")

...
async def create_device(body: DeviceCreate, current_user: _WriteDevice) -> DeviceCreatedOut:
...
async def get_device(device_id: str, current_user: _ReadDevice) -> DeviceOut:
    async with db.get_connection() as conn:
...
async def update_device(device_id: str, body: DeviceUpdate, current_user: _WriteDevice) -> DeviceOut:
    lat = body.location.lat if body.location else None
    lng = body.location.lng if body.location else None
    async with db.get_connection() as conn:
...
async def delete_device(device_id: str, current_user: _WriteDevice) -> None:
    async with db.get_connection() as conn:

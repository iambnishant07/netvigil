...
1. App calls `WebBrowser.openAuthSessionAsync(API_URL/auth/google/mobile, "netvigil://")`.
...
4. API exchanges the code for tokens using the web client secret (never exposed to the client), validates the `id_token`, finds/creates the user, and issues NetVigil JWTs.
5. API redirects to `netvigil://oauth-callback?access_token=...&refresh_token=...`.
...
- Adding `https://netvigil-api.up.railway.app/api/v1/auth/google/mobile-callback` to the Google Cloud Console authorized redirect URIs is a deployment pre-requisite.
